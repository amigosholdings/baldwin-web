...
test('writer prompt keeps evidence grounding while allowing useful synthesis', () => {
  const messages = [
    {
      role: 'system',
  ...
  assert.match(out[0].content, /synthesize across cited sources/);
  ...
  assert.match(out[1].content, /information gain/);
  assert.match(out[1].content, /1,300-2,200/);
});

test('editor remains strict without flattening article value', () => {
  const messages = [
    {
      role: 'system',
  ...
  assert.match(out[0].content, /preserve synthesis and practical implications/);
  assert.match(out[1].content, /clearer mental model/);
  assert.match(out[1].content, /so cautious that it no longer answers/);
});
