...
async function load() {
  ...
  try { GROWTH = await api('/v1/admin/agent/growth'); }
  catch (error) { GROWTH = { error: error.message }; }
  render();
  showApp();
  const expiry = localStorage.getItem('baldwin_agent_expires_at');
  status(`Loaded ${new Date(DATA.generated_at).toLocaleString()}${GROWTH?.error ? ' · Growth OS migration pending' : ''}${expiry ? ` · session expires ${new Date(expiry).toLocaleDateString()}` : ''}`);
}
...
function money(cents) { return '  const s = DATA.summary || {}, g = DATA.guardrails || {};
  const items = [
...
function renderGrowth() {
  if (!$('growthBudget')) return;
  if (!GROWTH || GROWTH.error) {
    $('growthBudget').innerHTML = '<div class="card"><b>Growth OS not initialized</b><div class="muted">Run worker migration v6, then refresh.</div></div>';
    $('growthExperimentRows').innerHTML = '<tr><td colspan="7">No portfolio data yet.</td></tr>';
    $('communityRows').innerHTML = '<tr><td colspan="6">No community opportunities yet.</td></tr>';
    return;
  }
  const b = GROWTH.budget || {};
  $('growthBudget').innerHTML = [
    ['Authorized', money(b.authorized_cents)], ['Spent', money(b.spent_cents)], ['Remaining', money(b.remaining_cents)]
  ].map(([k,v]) => `<div class="kpi"><b>${esc(k)}</b><span>${esc(v)}</span></div>`).join('');

  const variants = GROWTH.variants || [];
  const metrics = GROWTH.metrics_30d || [];
  const exps = GROWTH.experiments || [];
  $('growthExperimentRows').innerHTML = exps.length ? exps.map(e => {
    const ev = variants.filter(v => v.experiment_id === e.id);
    const em = metrics.filter(m => m.experiment_id === e.id);
    const totals = em.reduce((a,m) => {
      for (const k of ['app_signups','baseline_completes','second_sessions','subscription_starts']) a[k] += Number(m[k]||0);
      return a;
    }, {app_signups:0,baseline_completes:0,second_sessions:0,subscription_starts:0});
    return `<tr><td><b>${esc(e.name)}</b><div class="muted mono">${esc(e.id)}</div></td><td>${esc(e.channel)}</td><td>${esc(e.status)}</td><td>${money(e.spent_cents)} / ${money(e.spend_cap_cents)}</td><td>${ev.length}</td><td>${totals.baseline_completes}</td><td>${totals.subscription_starts}</td></tr>`;
  }).join('') : '<tr><td colspan="7">No experiments yet.</td></tr>';

  const opportunities = GROWTH.community_opportunities || [];
  $('communityRows').innerHTML = opportunities.length ? opportunities.map(x =>
    `<tr><td>${esc(x.platform)}</td><td>${esc(x.community || '')}</td><td><a href="${esc(x.source_url)}" target="_blank" rel="noopener">${esc(x.topic)}</a></td><td>${esc(x.intent || '')}</td><td>${esc(x.status)}</td><td><div class="copy">${esc(x.reply_draft || '')}</div></td></tr>`
  ).join('') : '<tr><td colspan="6">No queued public-thread opportunities.</td></tr>';
}
